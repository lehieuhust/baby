...
do
    ...
    # has to be correct balance of denom ubaby
    BALANCE=$(curl -X GET "http://localhost:$NODE_PORT/cosmos/bank/v1beta1/balances/$account" -H  "accept: application/json" | jq '.balances[] | select(.denom | . == "ubaby") | .amount' -r)
